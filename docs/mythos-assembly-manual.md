...
```mermaid
graph TD
  A[Movement Ritual] --> B[Rosetta Console]
  B --> C[Reason Receipt JSON]
  C --> D[Sentient Cents Ledger]
  D --> E[Badge Minting]
  E --> F[ScrollOS Publishing]
  F --> G[Prima Trust Dashboard]
  G --> H[Audit Trail]
```
...
```mermaid
graph LR
  Ritual --> Badge --> Ledger --> Scroll --> Product --> Treasury
```
