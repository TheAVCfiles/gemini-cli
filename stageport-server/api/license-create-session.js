...
export default async function handler(req, res) {
  try {
    ...
    const { facultyId, licenseType = 'institutional', institution = 'Unknown', auditHash } = req.body;
    if (!facultyId || !auditHash) return res.status(400).json({ error: 'facultyId and auditHash required' });
