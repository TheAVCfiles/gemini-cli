...
export async function generateIdentity() {
  ...
  const privateJwk = await crypto.subtle.exportKey("jwk", signKeyPair.privateKey);
...
export async function verifyEnvelope(envelope, signatureB64Url, signingPublicKey) {
  const bytes = textEncoder.encode(JSON.stringify(envelope));
...
export async function postSnapshot({
  ...
  const signature = signingKeys?.privateKey ? await signEnvelope({ envelope, encrypted }, signingKeys.privateKey) : null;
...
export function subscribeWS({ wsUrl, token, onMessage, onOpen, onClose, onError }) {
