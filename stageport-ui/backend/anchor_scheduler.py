...
def detect_narrative_anchors(calendar_df, lookback_days=365):
    """Filter anchors in the recent lookback or next 90 days."""
...
def backtest_anchors(calendar_df, ledger_path):
    """Run signal check for each anchor and write backtest output."""
    ledger_df = pd.read_csv(ledger_path)
    results = []
    for _, anchor in calendar_df.iterrows():
        results.append(check_structural_bid_confirmation(ledger_df, anchor["anchor_date"]))
