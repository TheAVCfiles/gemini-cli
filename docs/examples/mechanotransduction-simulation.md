# Mechanotransduction Simulation Example

This self-contained Python example demonstrates a minimal, testable pipeline for tactile mechanotransduction:

1. Construct a tactile stimulus that combines flutter, buzz, pressure, and a transient tap.
2. Approximate mechanoreceptor transfer functions with FFT band-pass and low-pass filters.
3. Convert receptor envelopes into spikes with a leaky integrate-and-fire (LIF) model across a 10×10 fingertip grid.
4. Aggregate spikes into 50 ms windows to yield somatotopic heatmaps.
5. Compute coarse band-energy "percepts" and a toy decoder that selects the dominant receptor channel per window.

The script is intentionally lightweight so you can iterate on parameters (frequencies, amplitudes, time constants, pooling windows, etc.)
to explore hypotheses about tactile texture encoding.

## Requirements

The example uses `numpy` and `matplotlib`. Install them with:
...
Save the following code to a notebook or Python file (for example, `mechanotransduction_sim.py`) and run it with Python 3. The script
renders the stimulus, spike raster, spatial heatmaps, and decoder readouts.
...
- Adjust `win_ms` to study the effect of cortical pooling on spatial acuity.
- Swap in empirically measured transfer functions in place of the crude FFT masks.
- Replace the toy decoder with a classifier that maps the spike maps to texture labels or materials.
- Extend the receptor grid to model curved surfaces or non-uniform receptor density.
