...
const callGemini = async (prompt, systemInstruction = '') => {
  try {
    ...
    return (
      data.candidates?.[0]?.content?.parts?.[0]?.text || 'The Signal is silent.'
    );
  } catch (error) {
...
const StrawberryField = () => (
  <div className="absolute top-10 right-10 w-64 h-64 pointer-events-none opacity-20 mix-blend-screen">
    <div
      className="absolute inset-0 bg-rose-500/30 blur-[80px] animate-pulse"
      style={{ animationDuration: '4s' }}
    />
    <div
      className="absolute top-1/4 left-1/4 w-32 h-32 bg-cyan-500/20 blur-[60px] animate-bounce"
...
const MechanicalUnderwire = ({ isVisible }) => (
  <div
    className={`fixed inset-0 pointer-events-none z-[0] transition-opacity duration-1000 ${isVisible ? 'opacity-20' : 'opacity-[0.03]'}`}
  >
    ...
    <div className="absolute top-[85px] left-[100px] font-mono text-[8px] text-cyan-700 tracking-[0.2em]">
      SCHEMA: MoveMintEvent (v1.2)
    </div>
    <div className="absolute bottom-[45px] right-[50px] font-mono text-[8px] text-cyan-700 tracking-[0.2em]">
      LEDGER: 8F-992-DELTA
    </div>
...
const HearthCard = () => (
  <div className="p-6 border border-slate-800 bg-[#0a0a0f] relative group overflow-hidden">
    ...
    <div className="font-serif text-white text-lg italic mb-1">
      Sunday Gravy
    </div>
    <div className="text-[10px] font-mono text-slate-400 mb-4">
      Session ID: 8F-992-DELTA
    </div>
    ...
    <div className="space-y-2 border-l-2 border-slate-800 pl-3">
      ...
      <div className="flex justify-between text-[10px] font-mono text-slate-500">
        ...
        <span className="text-rose-400">3 (Minted)</span>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/95 backdrop-blur-xl p-8 sm:p-12 animate-in fade-in duration-300 overflow-y-auto">
      <div className="max-w-4xl mx-auto border border-cyan-900/30 p-8 relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-cyan-700 hover:text-cyan-400"
        >
          <X size={20} />
        </button>

        <div className="mb-8 border-b border-cyan-900/30 pb-4">
          <h2 className="font-mono text-2xl text-cyan-500 mb-2">
            System Underwire // Uploaded Specs
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 font-mono text-xs">
          <div className="space-y-6">
            <h3 className="text-white uppercase tracking-widest border-b border-white/10 pb-2">
              The Choreographic FSM
            </h3>
            <div className="p-4 bg-slate-900/50 border border-slate-800 text-slate-400 space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-rose-400">GLISSADE</span> → Prep / Timing
              </div>
              <div className="flex items-center gap-2">
                <span className="text-cyan-400">JETE</span> → Launch / Risk-On
              </div>
              <div className="flex items-center gap-2">
                <span className="text-amber-400">FERMATA</span> → Hold / Audit
                (Active)
              </div>
              <div className="flex items-center gap-2">
                <span className="text-emerald-400">CODA</span> → Settlement
              </div>
            </div>
...
export default function FacilityOS_Cathedral() {
  ...
  return (
    <div
      className={`min-h-screen ${THEME.bg} text-slate-200 font-sans selection:bg-rose-900/30 selection:text-white relative overflow-hidden flex`}
    >
      ...
      <TechSpecsOverlay
        ...
        onClose={() => setIsTechVisible(false)}
      />

      {/* LEFT RAIL */}
      <nav
        className={`w-[90px] border-r ${THEME.border} bg-[#080b10]/95 backdrop-blur-xl z-40 flex flex-col justify-between h-screen`}
      >
        <div>
          ...
          {[
            ...
            >
              <item.icon
                size={20}
                strokeWidth={1.5}
                className={`transition-colors ${view === item.id ? 'text-rose-200' : 'text-slate-600 group-hover:text-slate-400'}`}
              />
              <span
                className={`text-[8px] font-mono uppercase tracking-[0.2em] transition-colors ${view === item.id ? 'text-white' : 'text-slate-600'}`}
              >
                {item.label}
              </span>
              {view === item.id && (
                <div className="absolute left-0 top-0 h-full w-[2px] bg-rose-500" />
              )}
            </button>
          ))}
        </div>
        <div className="border-t border-slate-800">
          <button
            onClick={() => setIsTechVisible(!isTechVisible)}
            className="w-full h-[60px] flex items-center justify-center text-slate-600 hover:text-cyan-400 transition-colors"
          >
            <Code size={18} />
          </button>
        </div>
      ...
      <main className="flex-1 relative flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-12 pb-24 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {view === 'FOYER' && (
            <div className="grid grid-cols-12 gap-8 animate-in fade-in zoom-in duration-500">
              ...
              <div className="col-span-12 lg:col-span-8 p-10 border border-slate-800 bg-gradient-to-r from-[#0f1219] to-transparent relative overflow-hidden">
                <h2 className="text-4xl font-serif text-white italic mb-4">
                ...
                <p className="text-sm text-slate-400 max-w-lg mb-8 leading-relaxed">
                  ...
                  <span className="text-amber-400 font-mono">FERMATA</span>{' '}
                  state (Manual Review).
                  <br /> Ghost Labor Scanner detected 2.5hrs of unpaid admin
                  work.
                </p>
              ...
              <div className="col-span-12 lg:col-span-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 border border-slate-800 bg-[#0a0a0f] relative overflow-hidden">
                    ...
                    <div className="font-serif text-white text-lg mb-2">
                      Gala Roses
                    </div>
                    <div className="text-emerald-400 font-mono text-xs">
                      SPONSORED: Main St. Florist
                    </div>
                  </div>
          ...
          {/* PLACEHOLDERS */}
          {view === 'STUDY' && (
            <div className="flex items-center justify-center h-64 border border-dashed border-slate-800 text-slate-600 font-mono text-xs uppercase tracking-widest">
              Faculty Study Active
            </div>
          )}
          {view === 'BACKSTAGE' && (
            <div className="flex items-center justify-center h-64 border border-dashed border-slate-800 text-slate-600 font-mono text-xs uppercase tracking-widest">
              Backstage Vaults Secure
            </div>
          )}
        </div>
